...
where $\delta_{j}(t) := - \frac{\partial L_{t}}{\partial net_{j}(t)}$
...
internal state's error at time $t$ 
$\frac{\partial L_t}{\partial s_{c_{j}}^{\nu}(t)}$ is calculated 
...
$$
\delta_{\phi_{j}}(t) & = \frac{\partial L_t}{\partial y^{\phi_{j}}(t)} \frac{\partial y^{\phi_j}(t)}{\partial net_{\phi_{j}}(t)} \\
...
![f]()
%picture of memory block with peephole connections
...
\item 
\begin{enumerate}
\item Input gate activation $y^{in}$
\item Forget gate activation $y^{\phi}$
\item Cell input and cell state $s_{c}$
\end{enumerate}
\item Output gate activation $y^{out}$ and cell output $y^{c}$
\end{enumerate}

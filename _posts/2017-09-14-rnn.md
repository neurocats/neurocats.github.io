...
being specified, $ \phi $ is a function, $ x(t) $ is an 
...
next time step or you could have RNNs with recurrent 
...
represented by the graph in figure 4 and 5 mathematically. 
...
log-probabilities of each possible value of the discrete 
...
With that we get the following set of equations for t
ime-steps $t = 0,\ldots, \tau$:
...
$$ L = \sum_{t= 0}^{T}L_{t} = \sum_{t= 0}^{T} \frac{1}{2} \sum_{k:k ~ output ~ unit} (y^{k}_{t}-\hat{y}^{k}_{t})^{2}, $$

where $ y^k_t $ is the training target at time $t$.
...
$$ \frac{\partial L}{\partial \theta} = \sum_{t=0}^{T} \frac{\partial L_{t}}{\partial \theta} \\
 = \sum_{t=0}^{T} \sum_{k=0}^{t} (\frac{\partial L_{t}}{\partial h_{t}^{T}} \frac{\partial h_{t}}{\partial h_{k}^{T}} \frac{\partial^{+} h_{k}}{\partial \theta}) $$
